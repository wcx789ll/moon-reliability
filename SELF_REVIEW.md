...
| CI | Pass | `.github/workflows/test.yml` covers formatting, check, build, test, generated interfaces, benchmark, and example; the latest remote run for commit `339ba8b` passed |
...
- Mooncakes package `wcx789ll/moon-reliability:0.3.0` was published successfully with server status `200 OK`; the current `0.3.1` change is a CI/example/documentation release and should be published after the next push.
